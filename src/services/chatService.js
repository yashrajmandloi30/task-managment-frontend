...
export const getGroupMessages = async (groupId, page = 1) => {
  const response = await axiosInstance.get(`/api/chat/group/${groupId}?page=${page}`);
  return response.data;
};

// ✅ Get personal messages
export const getPersonalMessages = async (userId, page = 1) => {
  const response = await axiosInstance.get(`/api/chat/personal/${userId}?page=${page}`);
  return response.data;
};
...
export const markAsRead = async (groupId) => {
  const response = await axiosInstance.put('/api/chat/read', { groupId });
  return response.data;
};
